...
async function createCheckout(request,client,config){
  ...
  const session=await client.checkout.sessions.create({
    ...
    line_items:items.map(p=>({quantity:p.qty,price_data:{currency:'usd',unit_amount:p.price,tax_behavior:'exclusive',product_data:{name:`DNE / ${p.model} / ${p.size}`,description:'Top and bottom included.',images:[config.origin+'/'+p.image],metadata:{model:p.model,size:p.size,product_id:p.id}}}})),
